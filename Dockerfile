...
### -----------------------
FROM golang:1.21.2-bullseye AS development
...
WORKDIR /app
COPY . /app/
RUN go build -o bin/app cmd/sponsors-api/main.go
...
# The :debug image provides a busybox shell to enter (base-debian11 only, not static).
